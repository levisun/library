...
class Template
{
    ...
    private $includeFile = array(); // 记录所有模板包含的文件路径及更新时间
    protected $storage;
    ...
    public function fetch($template, $vars = array(), $config = array())
    {
        ...
        if (!empty($this->config['cache_id']) && $this->config['display_cache']) {
            // 读取渲染缓存
            $cacheContent = $this->cache->get($this->config['cache_id']);
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        // 添加安全代码及模板引用记录
        $content = '<?php if (!defined(\'ROOT_PATH\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . "\n" . $content;
        // 编译存储
        $this->write($cacheFile, $content);
    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if ($this->config['view_base']) {
                $module = isset($module) ? $module : $this->request->module();
                $path   = $this->config['view_base'] . ($module ? $module . DIRECTORY_SEPARATOR : '');
            } else {
                $path = isset($module) ? ROOT_PATH . $module . DIRECTORY_SEPARATOR . basename($this->config['view_path']) . DIRECTORY_SEPARATOR : $this->config['view_path'];
            }
        ...
        if (is_file($template)) {
            ...
            halt('template not exists:' . $template);
        }
    }
    ...
    public function write($cacheFile, $content)
    {
        ...
        if (false === file_put_contents($cacheFile, $content)) {
            halt('cache write error:' . $cacheFile);
        }
    }
